...
def test_production_bridge(cdp):
    result=ev(cdp,"(()=>{window.hasVerifiedTelemetry=()=>true;window.stockFor=()=>50000;window.findCommodity=name=>({name,quantity:50000,price_to_sell_to_base:777});window.priceSell=item=>item?Number(item.price_to_sell_to_base):null;if(typeof renderProductionModules!=='function'||!RHWV4.productionPricing)return{ok:false,reason:'bridge missing'};renderProductionModules();RHWV4.productionPricing.enhanceProduction();const cards=[...document.querySelectorAll('.production-card')];const card=cards.find(x=>x.querySelector('.production-title')?.textContent.trim()==='Reactor Systems');const button=card?.querySelector('.production-calc-button');if(!button)return{ok:false,reason:'button missing',cards:cards.map(x=>x.querySelector('.production-title')?.textContent.trim())};button.click();return{ok:true,hash:location.hash,ws:document.body.dataset.workspace,search:document.querySelector('#opsRecipeSearch')?.value||'',recipe:document.querySelector('#opsRecipe')?.selectedOptions?.[0]?.textContent||''}})()")
...
def test_calculator(cdp):
    ...
    selected=ev(cdp,"({recipe:opsRecipe.selectedOptions[0].textContent,count:document.querySelectorAll('[data-material-price]').length,cycle:document.querySelector('.ops-recipe-meta strong').textContent,suffix:[...document.querySelectorAll('.ops-price-input-wrap>span')].map(x=>x.textContent.trim()),alternative:[...document.querySelectorAll('*')].some(x=>x.children.length===0&&x.textContent.trim()==='ALTERNATIVE INPUTS')})")
    ...
    live=ev(cdp,"(()=>{RHWV4.state.calculator.materialPrices={};RHWV4.store.set(RHWV4.config.storageKeys.calculatorState,RHWV4.state.calculator);RHWV4.operations.renderCalculator();window.findCommodity=name=>({name,quantity:50000,price_to_sell_to_base:1234});window.priceSell=item=>item?Number(item.price_to_sell_to_base):null;RHWV4.productionPricing.refreshPricing();return{sources:[...document.querySelectorAll('.ops-price-source')].map(x=>x.textContent.trim()),values:[...document.querySelectorAll('[data-material-price]')].map(x=>x.value),saved:Object.keys(RHWV4.state.calculator.materialPrices||{}).length,total:opsTotalCost.textContent}})()")
    if not live["sources"] or any(x!="SOURCE // RHW LIVE BUY PRICE" for x in live["sources"]) or any(x!="1234" for x in live["values"]) or live["saved"]!=0 or "$" not in live["total"]:
        raise RuntimeError(f"Calculator RHW live-price fallback failed: {live}")

    manual=ev(cdp,"(()=>{const input=document.querySelector('[data-material-price]');const key=input.dataset.materialPrice;input.value='999';input.dispatchEvent(new Event('input',{bubbles:true}));RHWV4.productionPricing.refreshPricing();const row=input.closest('.ops-material-row');return{source:row.querySelector('.ops-price-source')?.textContent.trim()||'',value:input.value,saved:RHWV4.state.calculator.materialPrices?.[key]}})()")
    if manual.get("source")!="SOURCE // MANUAL" or manual.get("value")!="999" or manual.get("saved")!=999:
        raise RuntimeError(f"Calculator manual-price precedence failed: {manual}")

    fallback=ev(cdp,"(()=>{const input=document.querySelector('[data-material-price]');const key=input.dataset.materialPrice;input.value='';input.dispatchEvent(new Event('input',{bubbles:true}));RHWV4.productionPricing.refreshPricing();const row=input.closest('.ops-material-row');return{source:row.querySelector('.ops-price-source')?.textContent.trim()||'',value:input.value,saved:Object.prototype.hasOwnProperty.call(RHWV4.state.calculator.materialPrices||{},key)}})()")
    if fallback.get("source")!="SOURCE // RHW LIVE BUY PRICE" or fallback.get("value")!="1234" or fallback.get("saved"):
        raise RuntimeError(f"Calculator live fallback restore failed: {fallback}")
    ...
    ev(cdp,"(()=>{opsRecipeSearch.value='Superstr';opsRecipeSearch.dispatchEvent(new Event('input',{bubbles:true}));return{}})()"); time.sleep(.4)
    ev(cdp,"(()=>{document.querySelectorAll('[data-material-price]').forEach(i=>{i.value='1000';i.dispatchEvent(new Event('input',{bubbles:true}))});opsMargin.value='99';opsMargin.dispatchEvent(new Event('input',{bubbles:true}));RHWV4.productionPricing.refreshPricing();return{}})()"); time.sleep(.1)
    ...
    print("V4 interaction smoke passed: OPERATIONS calculator + RHW live pricing")
