...
Validation runs through `.github/workflows/rhw-pages-deploy.yml` and includes structural checks, JavaScript syntax validation, Discovery sync unit coverage, full headless-Chrome route/interactions, PWA install/offline behavior, reliability diagnostics, corrupted-cache recovery, Clipboard truth-state, Production Order queue/material/Forum parity, Newswire 2.0 search/quality/channel-parity behavior and recipe-correctness coverage.
